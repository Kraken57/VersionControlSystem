...
struct File {
    ...
    // Add a constructor to initialize File objects
    File(const std::string& _name, bool _staged, const std::string& _content) : name(_name), staged(_staged), content(_content) {}
};
...
// Function to save VCS data to a custom file format
    void saveDataToFile() {
        std::ofstream file("vcs_data.txt");

        if (file.is_open()) {
            // Serialize VCS data and write it to the file
            // Example: Write branches, commits, and files data
            // You can customize this part based on your data structures

            // Write branches
            for (const Branch& branch : branches) {
                file << "Branch: " << branch.name << "\n";
                // Write head commit ID (if available)
                if (branch.headCommit) {
                    file << "HeadCommit: " << branch.headCommit->id << "\n";
                }
            }

            // Write commits
            for (const Commit& commit : commits) {
                file << "Commit: " << commit.id << "\n";
                file << "Message: " << commit.message << "\n";

                // Write committed files
                for (const File& committedFile : commit.files) {
                    file << "File: " << committedFile.name << "\n";
                    file << "Staged: " << (committedFile.staged ? "true" : "false") << "\n";
                    // Write file content
            ...
            file << "CurrentBranch: " << currentBranch->name << "\n";
    ...
    // Function to load VCS data from a custom file format
    void loadDataFromFile() {
        std::ifstream file("vcs_data.txt");

        if (file.is_open()) {
            // Deserialize VCS data and populate data structures
            // Example: Read branches, commits, and files data
            // You can customize this part based on your data structures
            ...
            Branch currentBranchTemp; // Temporary storage for the current branch
            Commit currentCommitTemp; // Temporary storage for the current commit
            File currentFileTemp;     // Temporary storage for the current file
            ...
            while (std::getline(file, line)) {
                ...
                if (token == "Branch:") {
                    // Read branch name
                    iss >> currentBranchTemp.name;
                } else if (token == "HeadCommit:") {
                    // Read head commit ID and link to the branch
                    std::string headCommitId;
                    iss >> headCommitId;
                    currentBranchTemp.headCommit = findCommitById(headCommitId);
                } else if (token == "Commit:") {
                    // Read commit ID
                    iss >> currentCommitTemp.id;
                } else if (token == "Message:") {
                    // Read commit message
                    std::getline(iss >> std::ws, currentCommitTemp.message);
                } else if (token == "File:") {
                    // Read committed file name
                    iss >> currentFileTemp.name;
                } else if (token == "Staged:") {
                    // Read staged status
                    std::string stagedStr;
                    iss >> stagedStr;
                    currentFileTemp.staged = (stagedStr == "true");
                } else if (token == "Content:") {
                    // Read file content
                    currentFileTemp.content = "";
                    while (std::getline(file, line)) {
                        if (line.empty()) {
                            break;
                        }
                        currentFileTemp.content += line + "\n";
                    }
                    // Add the current file to the current commit
                    currentCommitTemp.files.push_back(currentFileTemp);
                } else if (token == "CurrentBranch:") {
                    // Read the current branch name
                    std::string currentBranchName;
                    iss >> currentBranchName;
                    // Find and set the current branch by name
                    currentBranch = findBranchByName(currentBranchName);
                }
            }
            ...
            std::cerr << "Warning: Data file not found. Starting with an empty VCS." << std::endl;
        }
    }
...
private:
    // Helper function to find a commit by ID
    Commit* findCommitById(const std::string& commitId) {
        for (Commit& commit : commits) {
    ...
    Branch* findBranchByName(const std::string& branchName) {
        for (Branch& branch : branches) {
            if (branch.name == branchName) {
                return &branch;
            }
